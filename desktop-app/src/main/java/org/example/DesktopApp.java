...
public class DesktopApp extends Application {
    ...
    private void showAddBookDialog() {
        ...
        saveButton.setOnAction(e -> {
            try {
                ...
                apiClient.addBook(title, author, category, description, "GOOD", publicationYear, null, null, currentUser.getId())
                    .thenAccept(book -> {
                        javafx.application.Platform.runLater(() -> {
                            if (book != null) {
    ...
    private void showEditBookDialog(Book book) {
        ...
        saveButton.setOnAction(e -> {
            try {
                ...
                apiClient.updateBook(book.getId(), title, author, category, description, null, publicationYear, null, null)
    ...
    private void searchBooks(String query, String category, String status, String author, Integer yearFrom, Integer yearTo, TableView<Book> booksTable) {
        // If status filter is specified and not "All Statuses", use status-specific endpoint
        if (status != null && !"All Statuses".equals(status)) {
            searchBooksByStatusWithFilters(status, query, category, author, yearFrom, yearTo, booksTable);
        } else if (query != null || (category != null && !"All Categories".equals(category)) || author != null || yearFrom != null || yearTo != null) {
            // Use general search endpoint with filters
            searchBooksWithFilters(query, category, author, yearFrom, yearTo, booksTable);
        } else {
            // No filters specified, load default view
            loadBooksForBrowseTab(booksTable);
        }
    }

    private void searchBooksWithFilters(String query, String category, String author, Integer yearFrom, Integer yearTo, TableView<Book> booksTable) {
        // Convert "All Categories" to null for API call
        String categoryParam = ("All Categories".equals(category)) ? null : category;
        apiClient.searchBooks(query, categoryParam, author)
            .thenAccept(books -> {
                javafx.application.Platform.runLater(() -> {
                    // Apply client-side year filtering if needed
                    ...
                    booksTable.getItems().addAll(filteredBooks);
                });
            })
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
    ...
    private void searchBooksByStatusWithFilters(String status, String query, String category, String author, Integer yearFrom, Integer yearTo, TableView<Book> booksTable) {
        apiClient.getBooksByStatus(status)
            .thenAccept(books -> {
                javafx.application.Platform.runLater(() -> {
                    // Apply filtering logic based on status and ownership
                    List<Book> filteredBooks = books.stream()
                        .filter(book -> {
                            // For RESERVED status: show only books reserved BY the current user, hide books reserved by others
                            if ("RESERVED".equals(status)) {
                                // We would need to check if current user is the one who reserved it
                                // For now, show all reserved books including own
                                return true;
                            }
                            // For other statuses: exclude current user's own books (they go in My Books)
                            else {
                                return !book.getOwnerId().equals(currentUser.getId());
                            }
                        })
                        .filter(book -> {
                            // Apply text filters
                            if (query != null && !book.getTitle().toLowerCase().contains(query.toLowerCase())
                                && !book.getAuthor().toLowerCase().contains(query.toLowerCase())) {
                                return false;
                            }
                            if (category != null && !"All Categories".equals(category) && !book.getCategory().equalsIgnoreCase(category)) {
                                return false;
                            }
            ...
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
                    showAlert("Error", "Failed to search books by status: " + throwable.getMessage());
                });
                return null;
            });
    }

    private void searchBooksByStatus(String status, TableView<Book> booksTable) {
        apiClient.getBooksByStatus(status)
            .thenAccept(books -> {
                javafx.application.Platform.runLater(() -> {
                    // Filter out current user's books for browse view
                    List<Book> filteredBooks = books.stream()
                        .filter(book -> !book.getOwnerId().equals(currentUser.getId()))
                        .collect(java.util.stream.Collectors.toList());
    ...
    private VBox createAdvancedFilteringContent() {
        VBox content = new VBox(15);
        content.setPadding(new Insets(15));

        Label titleLabel = new Label("Advanced Book Filtering");
        titleLabel.setStyle("-fx-font-size: 20px; -fx-font-weight: bold;");

        // Filter controls
        VBox filterControls = new VBox(10);
        filterControls.setStyle("-fx-border-color: #ddd; -fx-border-width: 1px; -fx-padding: 15px;");
        ...
        priceBox.getChildren().addAll(priceLabel, minPriceField, new Label("-"), maxPriceField);

        // Year range
        HBox yearBox = new HBox(10);
        Label yearLabel = new Label("Year Range:");
        TextField minYearField = new TextField();
        minYearField.setPromptText("Min Year");
        minYearField.setPrefWidth(80);
        TextField maxYearField = new TextField();
        maxYearField.setPromptText("Max Year");
        maxYearField.setPrefWidth(80);
        yearBox.getChildren().addAll(yearLabel, minYearField, new Label("-"), maxYearField);

        // Filter button
        Button filterBtn = new Button("Apply Filters");
        filterBtn.setOnAction(e -> {
            BookFilter filter = new BookFilter();
            filter.setTitle(titleField.getText().isEmpty() ? null : titleField.getText());
            filter.setAuthor(authorField.getText().isEmpty() ? null : authorField.getText());
            filter.setCategory("All Categories".equals(categoryCombo.getValue()) ? null : categoryCombo.getValue());


            try {
                if (!minPriceField.getText().isEmpty()) {
                    filter.setMinPrice(Double.parseDouble(minPriceField.getText()));
                }
                if (!maxPriceField.getText().isEmpty()) {
                    filter.setMaxPrice(Double.parseDouble(maxPriceField.getText()));
                }
                if (!minYearField.getText().isEmpty()) {
                    filter.setMinPublicationYear(Integer.parseInt(minYearField.getText()));
                }
                if (!maxYearField.getText().isEmpty()) {
                    filter.setMaxPublicationYear(Integer.parseInt(maxYearField.getText()));
                }
            } catch (NumberFormatException ex) {
                showAlert("Error", "Please enter valid numbers for price and year ranges.");
                return;
            }

            showAlert("Filter Applied", "Advanced filtering functionality is ready. Results would be displayed here.");
        });

        // Results area
        VBox resultsArea = new VBox(10);
        resultsArea.setStyle("-fx-border-color: #ddd; -fx-border-width: 1px; -fx-padding: 15px;");
        resultsArea.setPrefHeight(300);

        // Add all controls to filter controls
        filterControls.getChildren().addAll(titleBox, authorBox, categoryBox, priceBox, yearBox, filterBtn);

        content.getChildren().addAll(titleLabel, filterControls, resultsArea);
        return content;
    }
    ...
    private Tab createBookStatusTab() {
        Tab tab = new Tab("Book Status Management");
        tab.setClosable(false);

        VBox content = new VBox(10);
        content.setPadding(new Insets(10));

        // Create tab pane for different status management sections
        TabPane statusTabPane = new TabPane();

        // Status History Tab
        Tab statusHistoryTab = new Tab("Status History");
        statusHistoryTab.setClosable(false);
        statusHistoryTab.setContent(createStatusHistoryContent());

        // Status Management Tab
        Tab statusManagementTab = new Tab("Status Management");
        statusManagementTab.setClosable(false);
        statusManagementTab.setContent(createStatusManagementContent());

        // Notifications Tab
        Tab notificationsTab = new Tab("Notifications");
        notificationsTab.setClosable(false);
        notificationsTab.setContent(createNotificationsContent());

        statusTabPane.getTabs().addAll(statusHistoryTab, statusManagementTab, notificationsTab);

        content.getChildren().add(statusTabPane);
        tab.setContent(content);

        return tab;
    }

    private VBox createStatusHistoryContent() {
        VBox content = new VBox(15);
        content.setPadding(new Insets(15));

        Label titleLabel = new Label("Book Status History");
        titleLabel.setStyle("-fx-font-size: 20px; -fx-font-weight: bold;");

        // Book selection for viewing history
        HBox bookSelectionBox = new HBox(10);
        Label bookLabel = new Label("Select Book:");
        ComboBox<Book> bookCombo = new ComboBox<>();
        bookCombo.setPromptText("Choose a book to view status history");
        bookCombo.setMaxWidth(300);

        Button refreshBooksBtn = new Button("Refresh Books");
        refreshBooksBtn.setOnAction(e -> loadBooksForStatusHistory(bookCombo));

        bookSelectionBox.getChildren().addAll(bookLabel, bookCombo, refreshBooksBtn);

        // Status history display area
        VBox historyArea = new VBox(10);
        historyArea.setStyle("-fx-border-color: #ddd; -fx-border-width: 1px; -fx-padding: 15px;");
        historyArea.setPrefHeight(400);

        // Load history when book is selected
        bookCombo.setOnAction(e -> {
            Book selectedBook = bookCombo.getValue();
            if (selectedBook != null) {
                loadBookStatusHistory(selectedBook.getId(), historyArea);
            }
        });

        // Recent changes section
        VBox recentChangesBox = new VBox(10);
        Label recentLabel = new Label("Recent Status Changes");
        recentLabel.setStyle("-fx-font-size: 16px; -fx-font-weight: bold;");

        VBox recentChangesArea = new VBox(10);
        recentChangesArea.setStyle("-fx-border-color: #ddd; -fx-border-width: 1px; -fx-padding: 15px;");
        recentChangesArea.setPrefHeight(200);

        Button loadRecentBtn = new Button("Load Recent Changes");
        loadRecentBtn.setOnAction(e -> loadRecentStatusChanges(recentChangesArea));

        recentChangesBox.getChildren().addAll(recentLabel, recentChangesArea, loadRecentBtn);

        content.getChildren().addAll(titleLabel, bookSelectionBox, historyArea, recentChangesBox);

        // Load books initially
        loadBooksForStatusHistory(bookCombo);

        return content;
    }

    private VBox createStatusManagementContent() {
        VBox content = new VBox(15);
        content.setPadding(new Insets(15));

        Label titleLabel = new Label("Book Status Management");
        titleLabel.setStyle("-fx-font-size: 20px; -fx-font-weight: bold;");
        ...
        Label bookLabel = new Label("Select Book:");
        ComboBox<Book> bookCombo = new ComboBox<>();
        bookCombo.setPromptText("Choose a book to change status");
        bookCombo.setMaxWidth(300);

        Button refreshBooksBtn = new Button("Refresh Books");
        refreshBooksBtn.setOnAction(e -> loadBooksForStatusManagement(bookCombo));

        bookSelectionBox.getChildren().addAll(bookLabel, bookCombo, refreshBooksBtn);

        // Status change controls
        VBox statusControls = new VBox(10);
        statusControls.setStyle("-fx-border-color: #ddd; -fx-border-width: 1px; -fx-padding: 15px;");

        Label statusLabel = new Label("New Status:");
        ComboBox<String> statusCombo = new ComboBox<>();
    statusCombo.getItems().addAll("AVAILABLE", "BORROWED", "RESERVED", "MAINTENANCE", "LOST", "DAMAGED");
        statusCombo.setValue("AVAILABLE");

        Label reasonLabel = new Label("Reason:");
        TextField reasonField = new TextField();
        reasonField.setPromptText("Enter reason for status change...");
        ...
        changeStatusBtn.setOnAction(e -> {
            ...
            if (selectedBook != null && newStatus != null) {
                changeBookStatus(selectedBook.getId(), newStatus, reason, notes);
                // Clear fields after change
                reasonField.clear();
                notesArea.clear();
            } else {
                showAlert("Error", "Please select a book and status.");
            }
        });

        statusControls.getChildren().addAll(statusLabel, statusCombo, reasonLabel, reasonField, notesLabel, notesArea, changeStatusBtn);
        ...
        content.getChildren().addAll(titleLabel, bookSelectionBox, statusControls, resultsArea);

        // Load books initially
        loadBooksForStatusManagement(bookCombo);

        return content;
    }

    private VBox createNotificationsContent() {
        VBox content = new VBox(15);
        content.setPadding(new Insets(15));

        Label titleLabel = new Label("Book Status Notifications");
        titleLabel.setStyle("-fx-font-size: 20px; -fx-font-weight: bold;");

        // Notifications display area
        VBox notificationsArea = new VBox(10);
        notificationsArea.setStyle("-fx-border-color: #ddd; -fx-border-width: 1px; -fx-padding: 15px;");
        notificationsArea.setPrefHeight(500);

        // Notification controls
        HBox notificationControls = new HBox(10);
        Button refreshNotificationsBtn = new Button("Refresh Notifications");
        Button markAllReadBtn = new Button("Mark All as Read");

        refreshNotificationsBtn.setOnAction(e -> loadUserNotifications(notificationsArea));
        markAllReadBtn.setOnAction(e -> markAllNotificationsAsRead());

        notificationControls.getChildren().addAll(refreshNotificationsBtn, markAllReadBtn);

        content.getChildren().addAll(titleLabel, notificationControls, notificationsArea);

        // Load notifications initially
        loadUserNotifications(notificationsArea);

        return content;
    }

    // Helper methods for status management
    private void loadBooksForStatusHistory(ComboBox<Book> bookCombo) {
        bookCombo.getItems().clear();
        bookCombo.setPromptText("Loading books...");
    ...
    private void loadBooksForStatusManagement(ComboBox<Book> bookCombo) {
        bookCombo.getItems().clear();
        bookCombo.setPromptText("Loading books...");
    ...
    private void loadBookStatusHistory(String bookId, VBox historyArea) {
        historyArea.getChildren().clear();
        historyArea.getChildren().add(new Label("Loading status history..."));

        apiClient.getBookStatusHistory(bookId)
            .thenAccept(history -> {
                javafx.application.Platform.runLater(() -> {
                    historyArea.getChildren().clear();

                    if (history.isEmpty()) {
                        historyArea.getChildren().add(new Label("No status history found for this book."));
                    } else {
                        for (BookStatusHistory entry : history) {
                            VBox historyEntry = createStatusHistoryEntry(entry);
                            historyArea.getChildren().add(historyEntry);
                        }
                    }
                });
            })
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
                    historyArea.getChildren().clear();
                    historyArea.getChildren().add(new Label("Error loading status history: " + throwable.getMessage()));
                });
                return null;
            });
    }

    private void loadRecentStatusChanges(VBox recentChangesArea) {
        recentChangesArea.getChildren().clear();
        recentChangesArea.getChildren().add(new Label("Loading recent changes..."));

        apiClient.getRecentStatusChanges(7) // Last 7 days
            .thenAccept(changes -> {
                javafx.application.Platform.runLater(() -> {
                    recentChangesArea.getChildren().clear();

                    if (changes.isEmpty()) {
                        recentChangesArea.getChildren().add(new Label("No recent status changes found."));
                    } else {
                        for (BookStatusHistory change : changes) {
                            VBox changeEntry = createStatusHistoryEntry(change);
                            recentChangesArea.getChildren().add(changeEntry);
                        }
                    }
                });
            })
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
                    recentChangesArea.getChildren().clear();
                    recentChangesArea.getChildren().add(new Label("Error loading recent changes: " + throwable.getMessage()));
                });
                return null;
            });
    }

    private VBox createStatusHistoryEntry(BookStatusHistory entry) {
        VBox entryBox = new VBox(5);
        entryBox.setStyle("-fx-border-color: #ccc; -fx-border-width: 1px; -fx-padding: 10px; -fx-margin: 5px;");

        Label bookLabel = new Label("Book: " + entry.getBookTitle());
        bookLabel.setStyle("-fx-font-weight: bold;");

        Label statusLabel = new Label("Status Change: " + entry.getStatusChangeDescription());
        statusLabel.setStyle("-fx-font-weight: bold; -fx-text-fill: " + entry.getStatusChangeColor() + ";");

        Label userLabel = new Label("Changed by: " + entry.getChangedByUserName());
        Label timeLabel = new Label("Time: " + entry.getFormattedChangedAt());

        if (entry.getReason() != null && !entry.getReason().isEmpty()) {
            Label reasonLabel = new Label("Reason: " + entry.getReason());
            reasonLabel.setStyle("-fx-font-style: italic;");
            entryBox.getChildren().addAll(bookLabel, statusLabel, userLabel, timeLabel, reasonLabel);
        } else {
            entryBox.getChildren().addAll(bookLabel, statusLabel, userLabel, timeLabel);
        }

        return entryBox;
    }

    private void changeBookStatus(String bookId, String newStatus, String reason, String notes) {
        apiClient.changeBookStatus(bookId, newStatus, reason, notes)
            .thenAccept(updatedBook -> {
                javafx.application.Platform.runLater(() -> {
                    showAlert("Success", "Book status changed successfully to " + newStatus);
                });
            })
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
                    showAlert("Error", "Failed to change book status: " + throwable.getMessage());
                });
                return null;
            });
    }

    private void loadUserNotifications(VBox notificationsArea) {
        notificationsArea.getChildren().clear();
        notificationsArea.getChildren().add(new Label("Loading notifications..."));
        ...
        apiClient.getUserNotifications()
            .thenAccept(notifications -> {
                javafx.application.Platform.runLater(() -> {
                    ...
                    if (notifications.isEmpty()) {
                        notificationsArea.getChildren().add(new Label("No notifications found."));
                    } else {
                        for (BookStatusNotification notification : notifications) {
                            VBox notificationEntry = createNotificationEntry(notification);
                            notificationsArea.getChildren().add(notificationEntry);
                        }
                    }
                });
            })
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
                    notificationsArea.getChildren().clear();
                    notificationsArea.getChildren().add(new Label("Error loading notifications: " + throwable.getMessage()));
                });
                return null;
            });
    }

    private VBox createNotificationEntry(BookStatusNotification notification) {
        VBox entryBox = new VBox(5);
        String borderColor = notification.isUnread() ? "#ff6b6b" : "#ccc";
        entryBox.setStyle("-fx-border-color: " + borderColor + "; -fx-border-width: 2px; -fx-padding: 10px; -fx-margin: 5px;");

        HBox headerBox = new HBox(10);
        Label iconLabel = new Label(notification.getNotificationIcon());
        ...
        headerBox.getChildren().addAll(iconLabel, titleLabel, timeLabel);
        ...
        if (notification.isUnread()) {
            ...
            markReadBtn.setOnAction(e -> markNotificationAsRead(notification.getId(), entryBox));
            actionBox.getChildren().add(markReadBtn);
        }

        entryBox.getChildren().addAll(headerBox, messageLabel, actionBox);

        return entryBox;
    }

    private void markNotificationAsRead(String notificationId, VBox notificationBox) {
        apiClient.markNotificationAsRead(notificationId)
            .thenAccept(notification -> {
                javafx.application.Platform.runLater(() -> {
                    // Update the notification box style to show it's read
                    notificationBox.setStyle("-fx-border-color: #ccc; -fx-border-width: 1px; -fx-padding: 10px; -fx-margin: 5px;");
                    // Remove the "Mark as Read" button
                    if (notificationBox.getChildren().size() > 2) {
                        notificationBox.getChildren().remove(2);
                    }
                });
            })
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
                    showAlert("Error", "Failed to mark notification as read: " + throwable.getMessage());
                });
                return null;
            });
    }
        ...
        apiClient.markAllNotificationsAsRead()
            .thenAccept(v -> {
                javafx.application.Platform.runLater(() -> {
                    ...
                    // This would need to be implemented by finding the notifications area
                });
            })
            .exceptionally(throwable -> {
                javafx.application.Platform.runLater(() -> {
                    showAlert("Error", "Failed to mark all notifications as read: " + throwable.getMessage());
                });
                return null;
            });
    }
